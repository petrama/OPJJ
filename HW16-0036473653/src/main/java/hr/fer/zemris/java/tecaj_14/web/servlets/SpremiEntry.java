...
public class SpremiEntry extends HttpServlet {
		...
		private void obradi(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
			...
			BlogEntryForm f=new BlogEntryForm().popuniIzHttpRequesta(req);
			
			f.validiraj();
			
			if(f.imaPogresaka()){
				req.setAttribute("form", f);
				req.getRequestDispatcher("/WEB-INF/pages/FormularPosta.jsp").forward(req, resp);
				return;
			}
