...
public class Mask {
	 ...
	 * Stvara masku koja nije dont-care vrijednosti.
	 ...
	 * Kopira ulazne vrijednosti i stavlja ih u privatnu listu.
	 * @param val  ulazne vrijednosti
	 * @param dont zastavica koja ako je true oznacava da je nova maska dont-care.
	 */
	public Mask(MaskValue[] val,boolean dont){
		values = Arrays.asList(val);
		dontCare=Boolean.valueOf(dont);
	}
	/**
	 * Metoda koja određuje da li maska predstavlja dont-care vrijednost.
	 * @return true ako je maska dont-care, false inače.
	 */
	...
	public static Mask combine(Mask prvaMaska, Mask drugaMaska) {
		  ...
		  if (!kompatibilni) {
		   ...
		   return new Mask(pomocna,prvaMaska.isDontCare()&&drugaMaska.isDontCare());
	...
	public static Mask fromIndex(int numOfVariables, int number) {
		...
		return parse(sb.toString() + binaryNumber);
	}
	...
	public List<MaskValue> getValues() {
		return values;
	}
